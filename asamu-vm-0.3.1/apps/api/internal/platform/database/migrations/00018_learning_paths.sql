...
CREATE INDEX learning_paths_public_idx ON learning_paths(status,sort_order,title);
...
CREATE INDEX learning_stages_path_idx ON learning_stages(path_id,sort_order);

CREATE TABLE learning_stage_challenges (
...
CREATE INDEX learning_stage_challenges_challenge_idx ON learning_stage_challenges(challenge_id);

-- +goose Down
DROP TABLE IF EXISTS learning_stage_challenges,learning_stages,learning_paths CASCADE;
